...
export function toBrDateString(date, outputHours = false, isoFormat = false) {
  ...
  const dateCompensatingTimezone = new Date(
    dateObj.getTime() - 3 * 60 * 60 * 1000
  );
  const year = dateCompensatingTimezone.getUTCFullYear();
  const month = String(dateCompensatingTimezone.getUTCMonth() + 1).padStart(
    2,
    "0"
  );
  const day = String(dateCompensatingTimezone.getUTCDate()).padStart(2, "0");
  let finalStr = isoFormat
    ? `${year}-${month}-${day}`
    : `${day}/${month}/${year}`;
  if (outputHours) {
    const hour = String(dateCompensatingTimezone.getUTCHours()).padStart(
      2,
      "0"
    );
    const minute = String(dateCompensatingTimezone.getUTCMinutes()).padStart(
      2,
      "0"
    );
...
export function parteEhEmbasa(nome) {
  return nomesEmbasa.some(nomeEmbasa =>
    nome.toLowerCase().includes(nomeEmbasa.toLowerCase())
  );
}
