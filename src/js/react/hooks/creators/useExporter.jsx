...
export default function useExporter(msgSetter) {
  ...
  async function processoAlreadyExists(workbookId, numeroProcesso) {
    ...
    const matchingRows = await loadSheetRange(
      "processos",
      null,
      { operator: operators.insensitiveStrictEquality, val: numeroProcesso },
      false,
      false,
      workbookId
    );
  ...
  async function writeMatriculaToSheet(espaiderMatricula, sheetId, token) {
    const values = espaiderMatricula.map(
      ({
        numeroProcesso,
        matricula,
        negativacao,
  ...
  async function writeLitisconsortesToSheet(
    espaiderPartes,
    hasCpfCnpj,
    sheetId,
    token
    ...
    const params = {
      ...
      sheetId,
      token,
      sheetName: hasCpfCnpj
        ? hardcoded.cadastroLitisconsortes
        : hardcoded.cadastroLitisconsortesSemCpf,
      msgs: {
  ...
  async function writeAndamentosToSheet(espaiderAndamentos, sheetId, token) {
    const values = espaiderAndamentos.map(
      ({ id, numeroProcesso, numeroDesdobramento, nome, data, obs }) => {
  ...
  async function writeProvidenciasToSheet(
    espaiderProvidencias,
    sheetId,
    token
  ) {
    const values = espaiderProvidencias.map(
      ({
        ...
        obs,
        gerarAndamento,
        codigoDatabaseAndamentoParaGerar,
  ...
  function registerProvidenciasInGroups(relevantProvidenciasData, token) {
    const registerableProvidencias = relevantProvidenciasData.filter(
      providencia => !!providencia.sheetName
    );
    registerableProvidencias.forEach(async providenciaData => {
      ...
      const { cellInA1Notation, value } = getCellAndValue(
        date,
        responsavelName,
        fullSheetDataJson.values
      );
      const rangeName = `${sheetName}!${cellInA1Notation}`;
      const valueToWrite = [[value + 1]];
      await writeToRange(
        googleUrls.configSheetId,
        rangeName,
        valueToWrite,
        token,
        false
  ...
  function getCellAndValue(date, responsavelName, sheetData) {
    const responsaveisRowData = sheetData.filter(
      row => String(row[0]).toLowerCase() === "responsáveis"
    );
    const columnIndex = responsaveisRowData[0].findIndex(
      entry => entry === responsavelName
    );
    const dateStr = new Date(date).toLocaleDateString("pt-BR");
    const rowIndex = sheetData.findIndex(
      row => String(row[0]).toLowerCase() === dateStr
    );
    ...
    const values = espaiderPedidos.map(
      ({
        ...
        return [
          ...
          nome,
          valor,
          databaseAtualizacaoString,
          riscoOriginal,
          riscoBaseadoEm,
          valorRiscoOriginal,
          estimativaPagamentoString,
          indiceAtualizacao,
          databaseJurosString,
          porcentagemJuros,
        ];
  ...
  async function writeEntitiesToSheet(creationParams) {
    ...
    const jSonResponse = await appendToSheet(
      sheetId,
      sheetName,
      finalArray,
      token
    ...
    if (!creationSuccessful) {
      //(requestSuccessful && creationSuccessful)) {
    ...
    msgSetter.addMsg({
      type: "success",
      msg: `${createdRowsAmount}/${qtd} ${msgs.success}`,
    });
  }
