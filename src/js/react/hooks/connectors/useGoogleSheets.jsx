...
export async function fetchGoogleSheetRowsMatchingExpression(
  sheetName,
  expressionToSearch,
  token,
  getMany
...
export default function useGoogleSheets() {
  ...
  function requestGoogleSheetContents(sheetInfo, token) {
    ...
    const range =
      sheetInfo.name + (sheetInfo.range ? `!${sheetInfo.range}` : "");
    const queryString = sheetInfo.readByColumns
  ...
  async function getGeneratedSheetsInfo(date, token) {
    if (!token) token = await fetchGoogleToken();
  ...
  async function getFileId(fileName, token) {
    ...
    if (json.files && json.files.length > 1)
      return `There are multiple files with name ${fileName}`;
  ...
  async function loadSheetRange(
    ...
    const responseJson = await fetchGoogleSheetData(
      sheetName,
      null,
      workbookId,
      rangeName,
    ...
    if (filterObject) {
      filteredOptions = filteredOptions.filter(option => {
        const valuesToSearch = Array.isArray(filterObject.val)
          ? filterObject.val
          : [filterObject.val];
